...
module.exports = function(feature, layer) {
  var content = '<div><ul>';
  var attribute, li = '', title, val, aCls;
          ...
          if(feature.get(attribute['name'])) {
            ...
            if (attribute['url']) {
              if(feature.get(attribute['url'])) {
                ...
                var url = createUrl(attribute['urlPrefix'], attribute['urlSuffix'], replacer.replace(attribute['url'], feature.getProperties()));
              }
              val = '<a href="' + url + '" target="_blank" class="' + aCls + '">' + feature.get(attribute['name']) + '</a>';
            }
          }
        }
        else if (attribute['url']) {
          if(feature.get(attribute['url'])) {
            var text = attribute['html'] || feature.get(attribute['url']);
            var url = createUrl(attribute['urlPrefix'], attribute['urlSuffix'], replacer.replace(feature.get(attribute['url']), feature.getProperties()));
            val = '<a href="' + url + '" target="_blank" class="' + aCls + '">' + text + '</a>';
          } else {
