...
const attachmentclient = function attachmentclient(layer) {
  ...
  function getId(feature) {
    if (config.foreignKey) {
      ...
      if (typeof feature.get(config.foreignKey) === 'undefined' && config.foreignKey.indexOf('.') > 0) {
        const splitMatch = config.foreignKey.split('.');
        ...
        foreignKeyValue = objectTemp; // Assign the nested value to featureValue
      } else {
        foreignKeyValue = feature.get(config.foreignKey);
      }
      return encodeURIComponent(foreignKeyValue);
    }
