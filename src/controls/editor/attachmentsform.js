 ...
 */
const attachmentsform = function attachmentsform(layer, feature, el, opts = {}) {
  const {
    localizeFunc
  } = opts;
  ...
  el.innerHTML = '';
  el.appendChild(createElement('label', `${layer.get('attachments').formTitle || localizeFunc('attachmentsTitle')}`));
  el.appendChild(document.createElement('br'));
  el.appendChild(createElement('p', localizeFunc('fetchAttachments')));
  ...
  ac.getAttachments(feature).then(data => {
    ...
    groups.forEach(group => {
      ...
      if (data.has(group.name)) {
        ...
        value.forEach(link => {
          ...
          deleteButtonEl.addEventListener('click', () => {
            if (window.confirm(`${localizeFunc('deleteConfirmFile')} ${link.filename}`)) {
              ac.deleteAttachment(feature, link.id)
                .then(() => {
                  ...
                  attachmentsform(layer, feature, el, { localizeFunc });
                })
                .catch(err => {
                  const errorMessageEl = createElement('p', `${localizeFunc('deleteFailedFile')}: ${err}`);
      ...
      const labelEl = createElement('label', `<span data-tooltip="${localizeFunc('addButton')}"></span>`, { style: 'cursor: pointer;', cls: 'hover o-tooltip' });
      const addButtonEl = createElement('button', '<span class="icon"><svg class="o-icon-24"><use xlink:href="#ic_add_24px"></use></svg></span>', { cls: 'compact', 'aria-label': localizeFunc('addButton'), style: 'pointer-events:none;' });
      // Do some silly css stuff to hide the actual file input as it is very ugly and use the label instead
      const inputEl = createElement('input', '', { type: 'file', accept: `${acceptstring}`, cls: 'o-hidden-fileselector' });
      ...
      inputEl.addEventListener('change', ev => {
        ac.addAttachments(feature, ev.target.files, group.name)
          ...
          .catch(err => {
            const errorMessageEl = createElement('p', `${localizeFunc('addFailedFile')}: ${err}`);
            el.appendChild(errorMessageEl);
          });
      });
      el.appendChild(rowEl);
    ...
    .catch(err => {
      const errorMessageEl = createElement('p', `${localizeFunc('fetchFailedFile')}: ${err}`);
      el.appendChild(errorMessageEl);
    });
};
