      ...
      if (layer.getVisible()==true) {
        ...
        if (inSubgroup) {
          ...
          [].forEach.call(parentGroups, function(el) {
            if (!$(el).find('.o-icon-expand:first').hasClass('o-icon-expand-true')) {
              toggleGroup($(el).find('li.o-legend-header:first'));              
            }
          });

          toggleSubGroupCheck($('#' + name).parents('ul').has('.o-legend-header').first(), false);
        } else {
          $('#o-group-' + layer.get('group')).closest('.o-icon-expand').removeClass('o-icon-expand-false');
          $('#o-group-' + layer.get('group')).closest('.o-icon-expand').addClass('o-icon-expand-true');
